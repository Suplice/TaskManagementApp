...
function Task({ task, onTaskUpdated }) {
    ...
    if (isCompleted) {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL}/Task/Update`,
          updatedTask,
          {
            headers: {
  ...
  return (
    <div
      ...
      style={{ boxShadow: handleTaskBackgroundShadow() }}
    >
      ...
      <div className="TaskButtons">
        <button className="CompleteTaskButton" onClick={handleCompleteTask}>
          {" "}
          Complete{" "}
        </button>
        <button className="RemoveTaskButton" onClick={handleRemoveTask}>
          Remove
        </button>
      </div>
    </div>
